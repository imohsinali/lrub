...
describe("", function () {
  ...
  describe("User Functionalities", function () {
    ...
    it("should update land price and reject all other requests for the same land when accepting a request with bid price", async function () {
      ...
      await landRegistry.connect(witness).requestforBuyWithBid(landId, bidPrice);
      await landRegistry.connect(witness).requestforBuyWithBid(landId, bidPrice);

      let landRequest = await landRegistry.LandRequestMapping(1);
      let landRequest1 = await landRegistry.LandRequestMapping(2);
      let landRequest2 = await landRegistry.LandRequestMapping(3);

      console.log(landRequest.requestStatus,landRequest1.requestStatus,landRequest2.requestStatus)


      // Check that a new land request is created with correct properties
      const requestCount = await landRegistry.requestCount();
      console.log(requestCount);
      await landRegistry.connect(user1).acceptRequest(2);
      await landRegistry.connect(user1).acceptRequest(3);
      await landRegistry.connect(user1).acceptRequest(1);

       landRequest = await landRegistry.LandRequestMapping(1);
       landRequest1 = await landRegistry.LandRequestMapping(2);
       landRequest2 = await landRegistry.LandRequestMapping(3);


      console.log(landRequest.requestStatus,landRequest1.requestStatus,landRequest2.requestStatus)


      await landRegistry.connect(user3).makePayment(user1.address, 1, {
        from: user3.address,
        value: bidPrice,
      });
    ...
    it("should revert if incorrect payment amount is sent", async function () {
      // Accept the land request
      let bidPrice1 = ethers.utils.parseEther("7.9999");
      await landRegistry.connect(user3).requestforBuyWithBid(landId, bidPrice);

      await landRegistry.connect(user1).acceptRequest(1);

      await expect(
        landRegistry.connect(user3).makePayment(user1.address, 1, {
          from: user3.address,
          value: bidPrice1,
        })
      ).to.be.reverted;
