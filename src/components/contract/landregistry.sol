    ...
    if(lands[landId].ownerAddress == msg.sender && lands[landId].isLandVerified)
{
    ...
    function registerUser(bytes32  _name, bytes32  _dob, bytes32  _city,uint _cinc, string memory _document, string memory  _profilepic, bytes32  _email
    ) public {
        ...
        UserMapping[msg.sender] =User(msg.sender, _name, _dob, _city,_cinc, _document,_profilepic,_email,false,false, address(0),block.timestamp);
...
function verifyUser(address _userId) public {
    ...
    UserMapping[_userId].verfiedby=msg.sender;
    UserMapping[_userId].verifydate=block.timestamp;
...
function addLand(uint _area, bytes32 _landAddress,bytes32 _district, uint _landPrice, string memory  _allLatiLongi, uint _propertyPID, string memory  _document, string memory  _landpic) public {
    ...
    lands[landsCount] = Land(landsCount,_propertyPID, _area, _landPrice,_landAddress,_district, _allLatiLongi, _document, _landpic, false,false, payable(msg.sender),contractOwner  );
   ...
   numplots > 0&& newarea < lands[id].area &&!lands[id].isforSell &&!UserMapping[msg.sender].death)
{
    ...
    for (uint i = 1; i <= numplots && lands[id].area >= newarea; i++) {
        ...
        lands[landsCount] = Land(landsCount,subplotPID, newarea,lands[id].landPrice, lands[id].landAddress,lands[id].district, lands[id].allLatitudeLongitude,  lands[id].document, lands[id].Landpic,false ,true, payable(msg.sender), contractOwner);
        MyLands[msg.sender].push(landsCount);
        allLandList[1].push(landsCount);
        ...
        ) 
{
    ...
    function makeItforSell(uint _landId,bool sell,uint _newPrice  ) public {
